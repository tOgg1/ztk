...
ztk a          # alias
ztk ab         # alias
ztk absorb -u  # absorb and update PRs
```
...
| Flag | Description |
|------|-------------|
| `-u`, `--update` | Run `ztk update` automatically after absorbing |
...
# Let absorb figure out where each fix belongs and update PRs
ztk absorb -u
```
