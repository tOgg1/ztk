...
```bash
ztk modify
ztk modify -u   # modify and update PRs
```
...
| Flag | Description |
|------|-------------|
| `-u`, `--update` | Run `ztk update` automatically after modifying |
...
Use the `-u` flag to automatically sync to GitHub after modifying:
...
This combines `modify` + `update` into one step — perfect for quick fixes.
