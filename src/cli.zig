...
fn cmdModify(allocator: std.mem.Allocator, args: []const [:0]const u8) void {
    ...
    while (i > 0) {
        ...
        const desc = std.fmt.bufPrint(&desc_buf, "[{d}] {s}", .{ num, stk.commits[i].title }) catch continue;
        const desc_copy = allocator.dupe(u8, desc) catch continue;
...
fn cmdOpen(allocator: std.mem.Allocator) void {
    ...
    for (prs.items) |pr| {
        var desc_buf: [256]u8 = undefined;
        const desc = std.fmt.bufPrint(&desc_buf, "#{d} {s}", .{ pr.number, pr.title }) catch continue;
        const desc_copy = allocator.dupe(u8, desc) catch continue;
