...
pub fn formatCommentFull(
    ...
    var len = std.fmt.bufPrint(&buf, "- **Pull Request**: {s} (#{d})\n", .{ context.pr_title, context.pr_number }) catch &buf;
    try result.appendSlice(allocator, len);

    len = std.fmt.bufPrint(&buf, "- **Branch**: {s}\n", .{context.branch}) catch &buf;
    try result.appendSlice(allocator, len);

    if (comment.path) |path| {
        if (comment.line) |line| {
            len = std.fmt.bufPrint(&buf, "- **File**: {s}:{d}\n", .{ path, line }) catch &buf;
            try result.appendSlice(allocator, len);
        } else {
            len = std.fmt.bufPrint(&buf, "- **File**: {s}\n", .{path}) catch &buf;
            try result.appendSlice(allocator, len);
        }
    }
    len = std.fmt.bufPrint(&buf, "- **Reviewer**: {s}\n", .{comment.author}) catch &buf;
    try result.appendSlice(allocator, len);
...
pub fn formatReview(
    ...
    len = std.fmt.bufPrint(&buf, "- **Status**: {s}\n", .{r.state.toString()}) catch &buf;
    try result.appendSlice(allocator, len);
